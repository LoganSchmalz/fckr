...
We use EVM to protect file metadata by utilizing EVM ``portable'' signatures.
...
This occurs regardless of whether the file matches a rule in the IMA policy.
...
When EVM HMAC support is enabled, EVM may replace non-portable signatures with HMACs,
allowing file metadata to be modified at runtime (by design).
Thus, portable signatures are absolutely necessary on systems with HMAC support enabled;
they are also useful for moving files between systems,
as non-portable signatures include some system-specific metadata.
We found that disabling EVM HMAC support prevents Fedora from booting due to its reliance on dynamically-created files.
...
and IMA policies containing SELinux labels will fail to load if the SELinux policy is not loaded.
