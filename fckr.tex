...
Thus, LUKS containers can protect secret files when the system might boot in an unapproved state.
Compared to manual encryption and decryption of file contents, the kernel manages LUKS containers
and files are only decrypted in memory when accessed; % TODO : cite ?
LUKS containers also retain file metadata.
Whether a LUKS container or a manual encryption process is used, a decryption key must be kept in memory.
