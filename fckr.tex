...
LUKS containers can protect secret files when the system might boot in an unapproved state.
Compared to encrypting the contents of files manually, LUKS encryption is managed in the kernel
and file contents are automatically decrypted when accessed. % TODO : cite ?
Whether a LUKS container or a manual encryption process is used, an encryption key must be kept in memory.
Thus, we choose to rely on the widespread LUKS mechanisms, and additionally benefit from its ease of use.
