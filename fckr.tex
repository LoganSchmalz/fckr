...
At the same time, the Unified Extensible Firmware Interface (UEFI), the standard firmware platform for computers,
uses the TPM for Secure Boot to establish firmware and software integrity.
...
these kernel-level mechanisms can be used to enforce fine-grained access control over TPM-resident secrets.

In this work, we demonstrate how these existing security mechanisms can be combined along with
...
% ! Not technically a root of trust of measurement, that would be the UEFI
...
When the system boots, its UEFI measures the system firmware and the bootloader,
...
The following rules prevent even a root user from relabeling a malicious program to \exect, reading or removing protections from \filet files,
