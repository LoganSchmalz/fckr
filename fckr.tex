...
establishes three types:
\domaint for the \tpmsign executing domain, \exect for the \tpmsign binary label,
\filet for protected files.
% ? These can be included in the <file contexts> file if desired.
Note that some commonly used macros and attributes (e.g. \verb|files_type()| or \verb|file_type|) allow operations we do not want.
% TODO: cite https://unix.stackexchange.com/questions/729329/selinux-deny-access-to-my-custom-type-for-unconfined-t-processes
...
type tpm_sign_file_t; fs_associate(tpm_sign_file_t);
...
allow unconfined_t tpm_sign_t:process { transition siginh rlimitinh
  noatsecure };
...
as well as the \systemd init binary on the root filesystem.
...
\appendix
\section{Details of SELinux Policy}
