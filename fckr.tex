...
Most TPM-resident private keys are only usable within the TPM, so such keys will never be available on the operating system.\footnotemark
\footnotetext{
  Other secrets (including some keys, if desired) can be stored in the TPM and released to the operating system.
  We focus on keys that can only be used by the TPM.
}
