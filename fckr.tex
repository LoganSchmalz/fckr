...
We measure \systemd because, on Fedora, \systemd is responsible for loading the custom IMA and SELinux policies when the full system \verb|switch_root| occurs.
...
There are several distribution-specific details to consider when implementing these mechanisms.
...
Secondly, when EVM HMAC support is enabled, EVM may replace non-portable signatures with HMACs,
...
However, the built-in Fedora SELinux policy is not designed to support loading before \verb|switch_root|,
