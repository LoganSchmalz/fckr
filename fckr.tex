...
\footnotetext{
  ...
  Disabling the HMAC functionality causes issues in our Fedora environment.
}
...
We measure \systemd because, on Fedora, \systemd is responsible for loading the custom IMA and SELinux policies when the full system \verb|switch_root| occurs.\footnotemark
\footnotetext{
  We found loading IMA and SELinux policy earlier causes issues in our Fedora environment. 
...
\footnotetext{
  ...
  This is an implementation detail to support Secure Boot easily and does not affect the security of the setup,
...
If it fails, it will re-generate the primary key and then load \pk from its file.\footnotemark
\footnotetext{
  It is possible to make \pk as well as the administrator's public policy key permanently available in the TPM for a minor performance improvement.
}
For every use of \pk, it is necessary to follow the standard steps to verify a wildcard policy to authorize key usage \cite{tpm2-software:2026:policyauthorize}.
