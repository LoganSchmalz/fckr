Because trustworthiness of custom policies and signatures is required, enabling IMA and EVM can be complex.
By default, some distributions require an administrator to sign any custom IMA policies and public keys.
This involves adding a key certificate to \shim's Machine Owner Keys, then signing IMA policies and X.509 keys on the filesystem with the appropriate private key.
...
It is necessary to remove the \verb|ima-policy-load.sh| script from this module, since Fedora's SELinux policy is not designed to load during boot and our IMA policy (\zcref[S]{sec:implementation-ima}) relies on SELinux.
...
As discussed briefly in \zcref[S]{sec:implementation-evm}, sometimes it is necessary to enable EVM HMACs as well.
...
This can be done by running the following commands as the root user.
